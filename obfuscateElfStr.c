...
edit_dwarf2 (DSO *dso)
{
	...
	for (i = 1; i < dso->ehdr.e_shnum; ++i)
	{
		...
		if (strncmp (name, ".debug_", sizeof (".debug_") - 1) == 0)
		{
			for (j = 0; debug_sections[j].name; ++j)
				if (strcmp (name, debug_sections[j].name) == 0)
				{
					...
					fprintf(debug_fd, "Record debug section %d name %s data size %ld\n", i, name, data->d_size);
					break;
				}
		...
		else if (strncmp(name, ".strtab", sizeof(".strtab") - 1) == 0)
		{
			scn = dso->scn[i];
			data = elf_getdata (scn, NULL);
			debug_sections[DEBUG_STRTAB].data = data->d_buf;
			debug_sections[DEBUG_STRTAB].elf_data = data;
			debug_sections[DEBUG_STRTAB].size = data->d_size;
			debug_sections[DEBUG_STRTAB].sec = i;
